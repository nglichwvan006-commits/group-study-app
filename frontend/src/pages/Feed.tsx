...
const Feed: React.FC = () => {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 pb-20 pt-10 px-4">
      <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] shadow-2xl border border-white/20 dark:border-slate-800">
         ...
         <AnimatePresence>
            {posts.map((p) => (
               <motion.div 
                 ...
                 className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] shadow-xl border border-slate-100 dark:border-slate-800"
               >
                  <div className="flex justify-between items-start mb-4">
                     <Link to={`/profile/${p.user?.id}`} className="flex items-center gap-3">
                        ...
                        <div>
                           <h4 className="font-bold text-sm hover:underline text-slate-900 dark:text-white">{p.user?.name || 'Unknown'}</h4>
                           <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{p.user?.badge} • LV {p.user?.level}</p>
                        </div>
                     </Link>
                     {(p.userId === currentUser?.id || currentUser?.role === 'ADMIN') && (
                  ...
                  <div className="mt-6 pt-4 border-t border-slate-50 dark:border-slate-800 flex items-center gap-6">
                     ...
                     <button className="flex items-center gap-2 text-xs font-bold text-slate-400 hover:text-indigo-500 transition-colors"><MessageSquare size={16} /> Bình luận</button>
