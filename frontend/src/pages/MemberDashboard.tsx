...
import { BookOpen, MessageSquare, LogOut, Send, CheckCircle, FileText, Sun, Moon, Menu, X, Clock, Trophy, Bell, Sparkles, RefreshCw, Search, Users, UserCircle } from 'lucide-react';
        ...
        <div className="p-6">
          ...
          <div className="mt-8 p-4 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xl">
            <Link to={`/profile/${user?.id}`} className="block group">
               <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${getBadgeColor(user?.badge || 'Bronze')} flex items-center justify-center text-white mb-3 shadow-lg group-hover:scale-105 transition-all`}>
                  <Trophy size={20} />
               </div>
                            ...
                            <form onSubmit={handleSubmitAssignment} className="space-y-6">
                              <div className="flex flex-col gap-4">
                                <select 
                                  ...
                                  className="w-40 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2 text-xs font-bold"
                                >
                              ...
                              <button type="submit" disabled={isSubmitting} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-black py-4 rounded-2xl flex items-center justify-center gap-3 transition-all shadow-lg disabled:opacity-50">
