...
import { Search as SearchIcon, User, ChevronRight, Trophy, MessageSquare, Image as ImageIcon } from 'lucide-react';
...
const Search: React.FC = () => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      if (activeSearchTab === 'users') {
        ...
        // We need an API for searching posts. I'll assume one exists or create it.
        const res = await api.get(`/posts/feed?q=${query}`); // Reuse feed with query param
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-10 pt-10 pb-20 px-4">
      <div className="text-center space-y-4">
         <h2 className="text-4xl font-black tracking-tighter uppercase italic">Tìm kiếm cộng đồng</h2>
         <p className="text-slate-500 font-medium">Khám phá thành viên mới và những bài chia sẻ thú vị.</p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-[2.5rem] shadow-2xl border border-white/20 dark:border-slate-800">
         ...
         <form onSubmit={handleSearch} className="flex gap-3">
            ...
            <button className="px-10 bg-indigo-600 text-white rounded-2xl font-black hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-500/20 uppercase tracking-tighter">Tìm kiếm</button>
         </form>
      </div>
      ...
      <div className="space-y-6">
         ...
         {!isLoading && query && ((activeSearchTab === 'users' && userResults.length === 0) || (activeSearchTab === 'posts' && postResults.length === 0)) && (
            <div className="text-center py-20 text-slate-400 font-bold italic">Không tìm thấy kết quả nào phù hợp với "{query}".</div>
         )}
      </div>
    </div>
  );
